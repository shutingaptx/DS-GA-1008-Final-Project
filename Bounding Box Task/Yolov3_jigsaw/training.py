    ...
    for epoch in range(config["epochs"]):
        ...
        for step, samples in enumerate(dataloader):
            ...
            if step > 0 and step % 20 == 0:
                ...
                print('Epoch {}, Step {}, Loss {}'.format(epoch, step,_loss))
                logging.info(
            ...
            if step > 0 and step % 100 == 0:
                ...
                _save_checkpoint(net.state_dict(), config,epoch,step)
                # net.train(True)
...
def _save_checkpoint(state_dict, config, epoch, step, evaluate_func=None):
    # global best_eval_result
    checkpoint_path = config["sub_working_dir"] + str(epoch) + '_' + str(step) + "_model.pth"

    #checkpoint_path = os.path.join(config["sub_working_dir"], str(epoch), '_', str(step), "_model.pth")
