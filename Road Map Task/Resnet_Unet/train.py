...
from unet.resnet_unet_model import ResNet34Unet
...
from roadmap import masks_prep

dir_img = '../data/data'
annotation_csv = '../data/data/annotation.csv'
dir_checkpoint = 'checkpoints_0508_pre/'
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for sample, _, road_image in train_loader:
                ...
                imgs = F.interpolate(samples, size=(3,320,320))
                true_masks = masks_prep(road_image)
                # print(true_masks.shape)
                rs = []
                for mask in true_masks:
                   m = mask[0].int().numpy()
                   a = np.array(Image.fromarray(m).resize((320,320)))
                   s = torch.tensor(a).float()
                   rs.append(s)
                   #rs.append(torch.tensor(np.array(Image.fromarray(mask[0].int().numpy())).resize((400,400))).astype(int))
                true_masks = torch.stack(rs).unsqueeze_(1)
                # true_masks = batch['mask']
                #assert imgs.shape[2] == net.n_channels, \
                #    f'Network has been defined with {net.n_channels} input channels, ' \
                 #   f'but loaded images have {imgs.shape[2]} channels. Please check that ' \
                ...
                if global_step % max(2,((len(labeled_trainset)+len(labeled_valset)) // (10 * batch_size))) == 0:
                  ...
                  #      writer.add_histogram('grads/' + tag, value.grad.data.cpu().numpy(), global_step)
                    ...
                    if save_cp and val_score > cur_best_val:
                        try:
                        ...
                        torch.save(net.state_dict(),
                                   dir_checkpoint + f'unet_current_best.pth')
                        logging.info(f'Current bset model saved !')
                        cur_best_val = val_score

                    writer.add_images('images', imgs.reshape(B*N,C,H,W), global_step)
...
def get_args():
    parser = argparse.ArgumentParser(description='Train the Resnet_Unet on images and target masks',
...
if __name__ == '__main__':
    ...
    net = ResNet34Unet(num_classes=1, num_channels=3)
