...
# The dataset class for road map task
class RoadMapLabeledDataset(torch.utils.data.Dataset):
    def __init__(self, image_folder, annotation_file, scene_index, transform, extra_info=True):
        """
        Args:
            image_folder (string): the location of the image folder
    ...
    def __getitem__(self, index):
        ...
        # Resize all images to 800*918
        image_combine = torchvision.utils.make_grid(image_tensor, nrow=3, padding=0).numpy().transpose(1, 2, 0)
        image_resize = cv2.copyMakeBorder(image_combine, 144, 144, 0, 0, cv2.BORDER_CONSTANT, value=[0,0,0])
        image_out = image_resize.transpose(2,0,1)

        road_resize = np.zeros(image_resize.shape[:2])
        road_resize[:road_image.shape[0],:road_image.shape[1]] = road_image
        road_out = road_resize
        ...
        if self.extra_info:
            ...
            return torch.as_tensor(image_out), torch.as_tensor(road_out), extra

        else:
            # return image_tensor, road_image
            return torch.as_tensor(image_out), torch.as_tensor(road_out)
