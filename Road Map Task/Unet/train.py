...
dir_checkpoint = 'checkpoints_0502/'
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for sample, _, road_image in train_loader:
                ...
                imgs = F.interpolate(samples, size=(3,400,400))
                true_masks = masks_prep(road_image)
                # print(true_masks.shape)
                rs = []
                for mask in true_masks:
                   m = mask[0].int().numpy()
                   a = np.array(Image.fromarray(m).resize((400,400)))
                ...
                global_step += 1
                if global_step % max(20,((len(labeled_trainset)+len(labeled_valset)) // (10 * batch_size))) == 0:
